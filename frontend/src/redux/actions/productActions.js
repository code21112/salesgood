...
import {
    ...
    ALL_PRODUCTS_FAIL,
    GET_MAXPRICE_REQUEST,
    GET_MAXPRICE_SUCCESS,
    GET_MAXPRICE_FAIL,
...
export const getProducts = (keyword = '', currentPage = 1, price, category, rating = 0) => async (dispatch) => {
    try {
        ...
        // }
        // if (keyword && category) {
        //     link = `/api/v1/keyword=${keyword}&products?page=${currentPage}&price[lte]=${price[1]}&price[gte]=${price[0]}&category=${category}`
        // }
...
export const getMaxPrice = () => async (dispatch) => {
    try {
        dispatch({ type: GET_MAXPRICE_REQUEST })

        const { data } = await axios.get('/api/v1/products/maxprice')

        dispatch({
            type: GET_MAXPRICE_SUCCESS,
            payload: data
        })
    } catch (error) {
        dispatch({
            type: GET_MAXPRICE_FAIL,
            payload: error.response.data.message
        })
    }
}
